...
def _isattrib(obj, a):
    return a[0] != "_" \
           ...
           and a not in ("name")
#@-node:_isattrib
#@+node:class horarioCalendar
class horarioCalendar(object):
    """
    The horario calendar saves the load time of a horario.
    ...
    #@+node:__repr__
    def __repr__(self):
        return "<horarioCalendar %s>" % (str(self))
    #@-node:__repr__
    #@+node:add_load
    ...
    #@-others
#@-node:class horarioCalendar
#@+node:class _horarioBase
class _horarioBase(object):
    pass

#@-node:class _horarioBase
#@+node:class _Metahorario
class _Metahorario(type):
    doc_template = """
    A horario class. The horarios default attributes can
    ...
    @var vacation:
    Specifies the vacation of the horario. This attribute is
    ...
    def __init__(self, name, bases, dict_):
        super(_Metahorario, self).__init__(name, bases, dict_)
        self.name = name
        self.title = dict_.get("title", name)
        self._calendar = { None: horarioCalendar() }
        self._tasks = { }
        self.__set_vacation()
        self.__add_horario(bases[0])
    ...
    #@-node:__set_vacation
    #@+node:__add_horario
    def __add_horario(self, base):
        if issubclass(base, _horarioBase):
    ...
    def calendar(self, scenario):
        try:
            ...
            cal = self._calendar[scenario] = horarioCalendar(self._calendar[None])
            return cal
...
#@-node:class _Metahorario
#@+node:make_team
def make_team(horario):
    members = horario.get_members()
    if not members:
        return horario
...
class Booking(object):
    ...
    #@-others
#@-node:class Booking
#@+node:class horarioList
class horarioList(list):
    #@	@+others
    #@+node:__init__
...
#@-node:class horarioList
#@+node:class horario
class horario(_horarioBase):
    #@	<< declarations >>
    #@+node:<< declarations >>
    __metaclass__ = _Metahorario
    ...
    #@+node:_idendity_
    def _idendity_(cls):
        return "horario:" + cls.__name__

    _idendity_ = classmethod(_idendity_)
    #@-node:_idendity_
    #@+node:__repr__
    def __repr__(self):
        return "<horario %s>" % self.__class__.__name__
    #@-node:__repr__
    #@+node:__str__
    def __str__(self):
        return repr(self)
    ...
    #@+node:__or__
    def __or__(self, other):
        if type(other) is _Metahorario:
            other = other()

        result = horario()
        result._subhorario = _OrhorarioGroup(self, other)
        return result
    #@-node:__or__
    #@+node:__and__
    def __and__(self, other):
        ...
        result = horario()
        result._subhorario = _AndhorarioGroup(self, other)
        return result
    #@-node:__and__
    #@+node:_permutation_count
    def _permutation_count(self):
        if hasattr(self, "_subhorario"):
            return self._subhorario._permutation_count()

        return 1
    #@-node:_permutation_count
    #@+node:_get_horarios
    def _get_horarios(self, state):
        if hasattr(self, "_subhorario"):
            result = self._subhorario._get_horarios(state)

            if self.name != "horario":
                result.name = self.name

            if self.title != "horario":
                result.title = self.title

            return result

        result = horarioList(self)
        return result
    #@-node:_get_horarios
    #@+node:all_members
    def all_members(self):
        if hasattr(self, "_subhorario"):
            return self._subhorario.all_members()
    ...
    #@-others
#@-node:class horario
#@+node:class _horarioGroup


class _horarioGroup(object):
    #@	@+others
    #@+node:__init__
    def __init__(self, *args):
        self.horarios = []
    ...
    def __append(self, arg):
        if isinstance(arg, self.__class__):
            self.horarios += arg.horarios
            for r in arg.horarios:
                self._refactor(r)
            return
        elif isinstance(arg, horario):
            subhorarios = getattr(arg, "_subhorario", None)
            if subhorarios:
                self.__append(subhorarios)
                return
            else:
                self.horarios.append(arg)
        else:
            assert(isinstance(arg, _horarioGroup))
            self.horarios.append(arg)
    ...
    def __str__(self):
        ...
        return "(" + \
               string.join([str(r) for r in self.horarios],
                           " " + op + " ") + \
               ")"
    #@-node:__str__
    #@-others
#@-node:class _horarioGroup
#@+node:class _OrhorarioGroup


class _OrhorarioGroup(_horarioGroup):
    #@	@+others
    #@+node:_get_horarios
    def _get_horarios(self, state):
        for r in self.horarios:
            c = r._permutation_count()
            if c <= state:
                state -= c
            else:
                return r._get_horarios(state)

        assert(0)
    #@-node:_get_horarios
    #@+node:_permutation_count
    def _permutation_count(self):
        return sum([ r._permutation_count() for r in self.horarios])
    #@-node:_permutation_count
    #@-others
#@-node:class _OrhorarioGroup
#@+node:class _AndhorarioGroup


class _AndhorarioGroup(_horarioGroup):
    #@	@+others
    #@+node:__init__
    def __init__(self, *args):
        self.factors = [ 1 ]
        _horarioGroup.__init__(self, *args)
    #@-node:__init__
    #@+node:_refactor
    def _refactor(self, arg):
    ...
    #@-node:_refactor
    #@+node:_permutation_count
        #print "AndhorarioGroup", count, arg, self.factors
    ...
    #@+node:_get_horarios
    def _get_horarios(self, state):
        """delivers None when there are duplicate horarios"""
        result = []
        for i in range(1, len(self.factors)):
            f = self.factors[i]
            substate = state / f
            state %= f
            result.append(self.horarios[i - 1]._get_horarios(substate))

        result = horarioList(*list(utils.flatten(result)))
        dupl_test = { }
        for r in result:
    ...
    def _has_duplicates(self, state):
        horarios = self._get_horarios(state)
        tmp = { }
        for r in horarios:
            if tmp.has_key(r):
                return True

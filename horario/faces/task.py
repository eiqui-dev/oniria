...
def _build_balancing_list(tasks):
    ...
    If the tasks will allocate horarios in the sorting order of that list
...
def _as_string(val):
    ...
    if isinstance(val, horario.horario):
...
#@nonl
#@-node:Cache
#@+node:horario Allocators
#@+others
#@+node:VariableLoad
def VariableLoad(limit=0):
    """
    Allocates the horario with maximal possible load.
...
#@-node:VariableLoad
#@+node:_calc_load
def _calc_load(task, horario):
    #changed at the horario instance
    load = horario.__dict__.get("load")
    ...
    return min(task.load, task.max_load, horario.max_load or 100.0)
#@-node:_calc_load
#@+node:_calc_maxload
def _calc_maxload(task, horario):
    #changed at the horario instance
    max_load = horario.__dict__.get("max_load")
    if max_load: return max_load

    #an explicit load can overwrite max_load
    load = max(horario.__dict__.get("load", 0),
    ...
    #inherited by the horario
    max_load = horario.max_load
    if max_load: return max(max_load, load)
...
class AllocationAlgorithm(object):
    """This class is a base for horario allocation algorithms"""
    #@	@+others
    #@+node:test_allocation
    def test_allocation(self, task, horario):
        """This method simulates the allocation of a specific horario.
        It returns a list of values representing the state of the allocation.
        The task allocator calls test_allocation for every alternative horario.
        It compares the first items of all return lists, and allocates the
        horario with the minum first item value"""
        return (task.end, )
    #@-node:test_allocation
    #@+node:allocate
    def allocate(self, task, state):
        """This method eventually allocates a specific horario.
...
class StrictAllocator(AllocationAlgorithm):
    """This class implements the STRICT horario allocation"""
    #@	@+others
    #@+node:_distribute_len_loads
    def _distribute_len_loads(self, task, horario, effort, length):
        # A special load calculation, if effort and length are given.
        # and the horarios have a defined maxload, the load must be
        # individually calculated for each horario.

        # Formulars: r=horarios, t=task
        ...
        # to all horarios. If a horario has a max_load(r) < norm_load
        # the load of this horario will be max_load(r), and the other
        # horarios will have another (higher) norm_load

        max_loads = map(lambda r: (_calc_maxload(task, r), r), horario)
        max_loads.sort()

        efficiency_sum = sum(map(lambda r: r.efficiency, horario))
    ...
    def test_allocation(self, task, horario):
        ...
        loads = map(lambda r: (r, calc_load(r)), horario)
        ...
        if effort is not None:
            ...
            if length is not None:
                ...
                loads = self._distribute_len_loads(task, horario,
                                                   effort, length)
                def calc_load(res):
                    return loads[res]
            else:
                #the length depends on the count of horarios
        ...
        return (end, book_load), horario, calc_load, start, effort
    #@-node:test_allocation
    #@+node:allocate
    def allocate(self, task, state):
        # now really book the horario
        end_bl, horario, calc_load, start, effort = state
        end = end_bl[0]
        ...
        #    - booked_horario (to get the booked tasks)
        task.booked_horario = horario
        task.done = task.done
        task.todo = task.todo
    ...
    def balance(self, task, start, delta, adjust_date,
                calc_load, horario):
        book_load = max(map(lambda r: r.get_load(task.start, task.scenario), horario))
...
class SmartAllocator(StrictAllocator):
    ...
    def balance(self, task, start, delta, adjust_date,
        ...
        while True:
            #we have finished, when all horarios have the
            #same next free start date
            for r in horario:
                max_load = _calc_maxload(task, r)
                load = calc_load(r)

                #find the next free time of the horario
...
class SloppyAllocator(AllocationAlgorithm):
    ...
    def test_allocation(self, task, horario):
        if task.__dict__.has_key("effort"):
            return self.test_allocation_effort(task, horario)

        return self.test_allocation_length(task, horario)
    #@-node:test_allocation
    #@+node:test_allocation_length
    def test_allocation_length(self, task, horario):
        ...
        return -sum_effort, end, horario, intervals
    #@-node:test_allocation_length
    #@+node:test_allocation_effort
    def test_allocation_effort(self, task, horario):
        ...
        #intervals of each horario, and reduces
        ...
        while effort > 0:
            ...
            for r in horario:
                ...
                if book_load + load <= max_load:
                    horario_factor = book_load * r.efficiency
                    interval_horario.append((r, book_load, horario_factor))
                    factor += horario_factor
            ...
            if factor:
                ...
                intervals.append((date, book_end, length, interval_horario))

        return next_date, alloc_effort, horario, intervals
    #@-node:test_allocation_effort
    #@+node:allocate
    def allocate(self, task, state):
    ...
    def allocate_length(self, task, state):
        # now really book the horario
        neg_sum_effort, end, horario, intervals = state
    ...
    def allocate_effort(self, task, state):
        # now really book the horario
        end, effort, horario, intervals = state
        to_start = task._to_start
        to_end = task._to_end
        to_delta = task._to_delta
        ...
        for start, end, length, horarios in intervals:
            for r, load, factor in horarios:
...
class Task(object):
    ...
    can have other child tasks, or is a leaf of the tree. horarios
    ...
    @var length:
    Specifies the time the task occupies the horarios.  This is
    ...
    The format is: C{[ (horario, from, to, time), ... ]}
    ...
    @var performed_horario:
    The horarios who have already performed on the task. This attribute is readonly.


    @var balance:
    Specifies the horario allocation type. Possible values are
    CO{STRICT}, CO{SLOPPY}, CO{SMART}.

    @var horario:
    Specifies the possible horarios, that may be allocated for the
    task.

    @var booked_horario:
    Specifies the allocated horarios of a task. This attribute is
    readonly.

    @var load:
    Specifies the daily load of a horario for an allocation of the
    specified task. A load of 1.0 (default) means the horario is
    ...
    allocated tasks of a horario. A ME{max_load} of 1.0 (default)
    means the horario may be fully allocated. A ME{max_load} of 1.3
    means the horario may be allocated with 30% overtime.

    @var efficiency:
    The efficiency of a horario can be used for two purposes. First
    ...
    priority is more likely to get the requested horarios.  The
    ...
    __attrib_completions__ = { \
        ...
        "balance" : 'balance = ',
        "horario": 'horario = ',
        "performed"  : 'performed = [(|horario, "2002-02-01", "2002-02-05", "2H"),]',
        ...
        "#horario" : "get_horario_completions",
    ...
    def costs(self, cost_name, mode="ep"):
        """
        calculates the horario costs for the task.
        ...
        if 'e' in mode:
            costs += sum(map(lambda rl: getattr(rl[0], cost_name) * rl[1],
                             self._horario_length))

        if 'p' in mode:
            costs += sum(map(lambda rl: getattr(rl[0], cost_name) * rl[1],
                             self._performed_horario_length))
    ...
    #@-node:max
    #@+node:all_horarios
    def all_horarios(self):
        result = self._all_horarios_as_dict()
        result = result.keys()
        result.sort()
        return result
    #@-node:all_horarios
    ...
    def snapshot(self, indent="", name=None):
        ...
        for name in self._properties:
            ...
            val = getattr(self, name, None)
            try:
                if issubclass(val, horario.horario): continue
            except TypeError:
                pass
            text += "%s%s = %s\n" % (indent, name, _as_string(val))

        horarios = tuple(self._iter_booked_horarios())
        if horarios:
            text += "%shorario = \\\n" % indent
            def make_horario(res):
                return "%s    %s" \
                       % (indent, res.snapshot())

            text += "&\\\n".join(map(make_horario, horarios)) + "\n"

            def make_horario_booking(res):
            ...
            text += "%sperformed = [\n" % indent
            text += "\n".join(map(make_horario_booking, horarios)) + "]"
    ...
    def formatter(self, attrib_name, arg=None, format=None):
        ...
        if attrib_name in ("booked_horario", "performed_horario"):
            def get_horario_name(v):
                title = getattr(v, "title", None)
                if title: return title
                return ", ".join([r.title for r in v])
            return get_horario_name
    ...
    #@-node:formatter
    #@-node:Public methods
    #@+node:horario allocation Methods
    #@+node:_all_horarios_as_dict
    def _all_horarios_as_dict(self):
        if self.children:
            result = {}
            for c in self.children:
                result.update(c._all_horarios_as_dict())

            return result

        if self.horario:
            return dict(map(lambda r: (r, 1), self.horario.all_members()))

        return {}
    #@-node:_all_horarios_as_dict
    #@+node:_test_allocation
    def _test_allocation(self, horario_state, allocator):
        horario = self.horario._get_horarios(horario_state)
        if not horario:
            return False

        return allocator.test_allocation(self, horario)
    #@-node:_test_allocation
    #@+node:_allocate
    def _allocate(self, state, allocator):
        ...
        for r in self.performed_horario:
            r.correct_bookings(self)

        self._horario_length = map(lambda r: (weakref.proxy(r), \
                                               r.length_of(self)),
                                    self._iter_booked_horarios())
    #@-node:_allocate
    #@+node:_convert_performed
    def _convert_performed(self, all_horarios):
    ...
    """), "performed")
        ...
        def convert_item(index):
            ...
            if isinstance(res, str):
                found = filter(lambda r: r.name == res, all_horarios)
                if found: res = found[0]

            try:
                if not isinstance(res, (horario.horario,
                                        horario._Metahorario)):
                    raise ValueError("the horario '%s' is unknown." %  res)
    ...
    def _allocate_performed(self, performed):
        ...
        for r, v in summary.iteritems():
            ...
            #to display them correct in horario charts
        ...
        self._performed_horario_length = tuple([ (r, v[2]) for r, v in summary.iteritems() ])
        self.performed_horario = tuple(summary.keys())
    ...
    #@-node:_allocate_performed
    #@+node:_iter_booked_horarios
    def _iter_booked_horarios(self):
        result = dict(map(lambda r: (r, 1), self.performed_horario))
        result.update(dict(map(lambda r: (r, 1), self.booked_horario)))
    ...
    #@nonl
    #@-node:_set_performed_work_time
    #@+node:_set_booked_horario
    def _set_booked_horario(self, value):
        raise AttributeError("The attribute 'booked_horario' is readonly.")
    #@nonl
    #@-node:_set_booked_horario
    ...
    def _set_copy_src(self, value):
        ...
        if value.horario and not self.horario:
            self.horario = value.horario
    ...
    def __calc_done(self):
        ...
        res = self._iter_booked_horarios()
    ...
    def __calc_todo(self):
        ...
        todo = sum(map(lambda r: r.todo_of(self), self.booked_horario))
...
class _ProjectBase(Task):
    ...
    #@+node:free
    def free(self):
        all_horarios = self.all_horarios()
        for r in all_horarios:
    ...
    def snapshot(self, indent="", name=None):
        ...
        def make_horario(r):
            return '%sclass %s(horario): title = "%s"\n' \
                   % (indent, r.name, r.title)

        now = datetime.datetime.now().strftime("%x %H:%M")
        horario_text = map(lambda r: make_horario(r), self.all_horarios())
        lines.insert(1, "%sfrom faces import horario\n" % indent)
        lines.insert(2, "".join(horario_text) + "\n")
...
class Project(_ProjectBase):
    """
    Generates a Project without allocating horarios.
    ...
    other projects in the horario database. The default value for id
...
class BalancedProject(_AllocationPoject):
    """
    Generates a project with allocated horarios. The tasks are balanced
    to fit the horarios load conditions.
    """
    ...
    def allocate_snapshot(self):
        all_horarios = self.free()
        scenario = self.scenario
        has_actual_data = True
        for t in self:
            if not t.horario or t.milestone or t.children:
                continue

            t._convert_performed(all_horarios)
    ...
    def allocate(self):
        ...
        for t in balancing_list:
            ...
            if t._convert_performed(all_horarios):
                has_actual_data = True
            ...
            min_val = None
            min_state = None
            for p in range(t.horario._permutation_count()):
...
class AdjustedProject(_AllocationPoject):
    """
    Generates a project with allocated horarios. The tasks are
    adjusted to the actual tracking data and balanced to fit the
    horarios load conditions.
    """
    ...
    def allocate(self, base):
        ...
        #@    << free the horarios, we have to rebook >>
        #@+node:<< free the horarios, we have to rebook >>
        for t in balancing_list:
            src = base.get_task(t.path)
            if src.end > now or src.complete < 100:
                for r in src._iter_booked_horarios():
                    r.unbook_task(src)
        #@nonl
        #@-node:<< free the horarios, we have to rebook >>
        #@nl
        ...
        for t in balancing_list:
            ...
            if src.end <= now and src.complete == 100:
                ...
                t.done = src.done
                t.todo = src.todo
                t.booked_horario = src.booked_horario
                t.performed_horario = src.performed_horario
                t._unfreeze("length")
            ...
            if src.start >= now:
                ...
                horario = src.booked_horario or src.performed_horario
                state = allocator.test_allocation(t, horario)
                if state:

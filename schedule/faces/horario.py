...
class scheduleCalendar(object):
    ...
    #@+node:__repr__
    def __repr__(self):
        return "<scheduleCalendar %s>" % (str(self))
    #@-node:__repr__
    #@+node:add_load
    ...
    #@-others
#@-node:class scheduleCalendar
#@+node:class _scheduleBase
class _scheduleBase(object):
    pass
...
class _Metaschedule(type):
    ...
    def __init__(self, name, bases, dict_):
        super(_Metaschedule, self).__init__(name, bases, dict_)
        self.name = name
        self.title = dict_.get("title", name)
        self._calendar = { None: scheduleCalendar() }
        self._tasks = { }
        self.__set_vacation()
        self.__add_schedule(bases[0])
    ...
    #@-node:__set_vacation
    #@+node:__add_schedule
    def __add_schedule(self, base):
        if issubclass(base, _scheduleBase):
    ...
    def calendar(self, scenario):
        try:
            ...
            cal = self._calendar[scenario] = scheduleCalendar(self._calendar[None])
            return cal
...
#@-node:class _Metaschedule
#@+node:make_team
def make_team(schedule):
    members = schedule.get_members()
    if not members:
        return schedule
...
class schedule(_scheduleBase):
    ...
    #@+node:__or__
    def __or__(self, other):
        if type(other) is _Metaschedule:
            other = other()

        result = schedule()
        result._subschedule = _OrscheduleGroup(self, other)
        return result
    #@-node:__or__
    #@+node:__and__
    def __and__(self, other):
        ...
        result = schedule()
        result._subschedule = _AndscheduleGroup(self, other)
        return result
    #@-node:__and__
    #@+node:_permutation_count
    def _permutation_count(self):
        if hasattr(self, "_subschedule"):
            return self._subschedule._permutation_count()

        return 1
    #@-node:_permutation_count
    #@+node:_get_schedules
    def _get_schedules(self, state):
        if hasattr(self, "_subschedule"):
        ...
        result = scheduleList(self)
        return result
    #@-node:_get_schedules
    #@+node:all_members
    def all_members(self):
        if hasattr(self, "_subschedule"):
            return self._subschedule.all_members()
...
class _scheduleGroup(object):
    ...
    def __append(self, arg):
        if isinstance(arg, self.__class__):
            self.schedules += arg.schedules
            for r in arg.schedules:
                self._refactor(r)
            return
        elif isinstance(arg, schedule):
            subschedules = getattr(arg, "_subschedule", None)
            if subschedules:
                self.__append(subschedules)
                return
            else:
                self.schedules.append(arg)
        else:
            assert(isinstance(arg, _scheduleGroup))
            self.schedules.append(arg)
    ...
    def __str__(self):
        ...
        return "(" + \
               string.join([str(r) for r in self.schedules],
                           " " + op + " ") + \
               ")"
    #@-node:__str__
    #@-others
#@-node:class _scheduleGroup
#@+node:class _OrscheduleGroup


class _OrscheduleGroup(_scheduleGroup):
    #@	@+others
    #@+node:_get_schedules
    def _get_schedules(self, state):
        for r in self.schedules:
            ...
            if c <= state:
                ...
                return r._get_schedules(state)

        assert(0)
    #@-node:_get_schedules
    #@+node:_permutation_count
    def _permutation_count(self):
        return sum([ r._permutation_count() for r in self.schedules])
    #@-node:_permutation_count
    #@-others
#@-node:class _OrscheduleGroup
...
class _AndscheduleGroup(_scheduleGroup):
    #@	@+others
    #@+node:__init__
    def __init__(self, *args):
        self.factors = [ 1 ]
        _scheduleGroup.__init__(self, *args)
    #@-node:__init__
    #@+node:_refactor
    def _refactor(self, arg):
    ...
    #@-node:_refactor
    #@+node:_permutation_count
        #print "AndscheduleGroup", count, arg, self.factors
    ...
    #@+node:_get_schedules
    def _get_schedules(self, state):
        """delivers None when there are duplicate schedules"""
        result = []
        for i in range(1, len(self.factors)):
            f = self.factors[i]
            substate = state / f
            state %= f
            result.append(self.schedules[i - 1]._get_schedules(substate))

        result = scheduleList(*list(utils.flatten(result)))
        dupl_test = { }
        for r in result:
    ...
    def _has_duplicates(self, state):
        schedules = self._get_schedules(state)
        tmp = { }
        for r in schedules:
            if tmp.has_key(r):
                return True

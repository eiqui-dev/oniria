...
def _build_balancing_list(tasks):
    ...
    If the tasks will allocate schedules in the sorting order of that list
    ...
    def test_allocation(self, task, schedule):
        ...
        if effort is not None:
            ...
            if length is not None:
                ...
                #the length depends on the count of schedules
...
class SloppyAllocator(AllocationAlgorithm):
        ...
        while effort > 0:
            ...
            for r in schedule:
                ...
                if book_load + load <= max_load:
                    schedule_factor = book_load * r.efficiency
                    interval_schedule.append((r, book_load, schedule_factor))
    ...
    def allocate_effort(self, task, state):
        ...
        for start, end, length, schedules in intervals:
            for r, load, factor in schedules:
...
class Task(object):
    ...
    can have other child tasks, or is a leaf of the tree. schedules
    ...
    @var length:
    Specifies the time the task occupies the schedules.  This is
    ...
    @var performed_schedule:
    The schedules who have already performed on the task. This attribute is readonly.
    ...
    @var booked_schedule:
    Specifies the allocated schedules of a task. This attribute is
    readonly.
    ...
    priority is more likely to get the requested schedules.  The
    ...
    def costs(self, cost_name, mode="ep"):
        ...
        if 'e' in mode:
            costs += sum(map(lambda rl: getattr(rl[0], cost_name) * rl[1],
                             self._schedule_length))

        if 'p' in mode:
            costs += sum(map(lambda rl: getattr(rl[0], cost_name) * rl[1],
                             self._performed_schedule_length))
    ...
    def all_schedules(self):
        result = self._all_schedules_as_dict()
        result = result.keys()
        result.sort()
        return result
    #@-node:all_schedules
    ...
    def snapshot(self, indent="", name=None):
        ...
        schedules = tuple(self._iter_booked_schedules())
        if schedules:
            text += "%sschedule = \\\n" % indent
            def make_schedule(res):
                return "%s    %s" \
                       % (indent, res.snapshot())

            text += "&\\\n".join(map(make_schedule, schedules)) + "\n"

            def make_schedule_booking(res):
            ...
            text += "%sperformed = [\n" % indent
            text += "\n".join(map(make_schedule_booking, schedules)) + "]"
    ...
    def formatter(self, attrib_name, arg=None, format=None):
        ...
        if attrib_name in ("booked_schedule", "performed_schedule"):
            def get_schedule_name(v):
                title = getattr(v, "title", None)
                if title: return title
                return ", ".join([r.title for r in v])
            return get_schedule_name
    ...
    def _all_schedules_as_dict(self):
        if self.children:
            ...
            for c in self.children:
                result.update(c._all_schedules_as_dict())

            return result

        if self.schedule:
            return dict(map(lambda r: (r, 1), self.schedule.all_members()))
    ...
    def _allocate(self, state, allocator):
        ...
        for r in self.performed_schedule:
            r.correct_bookings(self)

        self._schedule_length = map(lambda r: (weakref.proxy(r), \
                                               r.length_of(self)),
                                    self._iter_booked_schedules())
    #@-node:_allocate
    #@+node:_convert_performed
    def _convert_performed(self, all_schedules):
    ...
    """), "performed")
        ...
        def convert_item(index):
            ...
            if isinstance(res, str):
                found = filter(lambda r: r.name == res, all_schedules)
    ...
    def _allocate_performed(self, performed):
        ...
        self._performed_schedule_length = tuple([ (r, v[2]) for r, v in summary.iteritems() ])
        self.performed_schedule = tuple(summary.keys())
    ...
    def _iter_booked_schedules(self):
        result = dict(map(lambda r: (r, 1), self.performed_schedule))
    ...
    #@nonl
    #@-node:_set_performed_work_time
    #@+node:_set_booked_schedule
    def _set_booked_schedule(self, value):
        raise AttributeError("The attribute 'booked_schedule' is readonly.")
    #@nonl
    #@-node:_set_booked_schedule
    ...
    def _set_copy_src(self, value):
        ...
        if value.schedule and not self.schedule:
    ...
    def __calc_done(self):
        ...
        res = self._iter_booked_schedules()
    ...
    def __calc_todo(self):
        ...
        todo = sum(map(lambda r: r.todo_of(self), self.booked_schedule))
...
class Project(_ProjectBase):
    """
    Generates a Project without allocating schedules.
...
class BalancedProject(_AllocationPoject):
    """
    Generates a project with allocated schedules. The tasks are balanced
    to fit the schedules load conditions.
    """
    ...
    def allocate_snapshot(self):
        all_schedules = self.free()
        scenario = self.scenario
        has_actual_data = True
        for t in self:
            if not t.schedule or t.milestone or t.children:
                continue

            t._convert_performed(all_schedules)
    ...
    def allocate(self):
        ...
        for t in balancing_list:
            ...
            if t._convert_performed(all_schedules):
                has_actual_data = True
            ...
            min_val = None
            min_state = None
            for p in range(t.schedule._permutation_count()):
...
class AdjustedProject(_AllocationPoject):
    """
    Generates a project with allocated schedules. The tasks are
    adjusted to the actual tracking data and balanced to fit the
    schedules load conditions.
    """
    ...
    def allocate(self, base):
        ...
        #@    << free the schedules, we have to rebook >>
        #@+node:<< free the schedules, we have to rebook >>
        for t in balancing_list:
            src = base.get_task(t.path)
            if src.end > now or src.complete < 100:
                for r in src._iter_booked_schedules():
                    r.unbook_task(src)
        #@nonl
        #@-node:<< free the schedules, we have to rebook >>
        #@nl
        ...
        for t in balancing_list:
            ...
            if src.end <= now and src.complete == 100:
                ...
                t.done = src.done
                t.todo = src.todo
                t.booked_schedule = src.booked_schedule
                t.performed_schedule = src.performed_schedule
                t._unfreeze("length")
